...
$expected = @(
    ...
    [ordered]@{ key = 'manual_calculation_result'; automation_id = '334'; name = 'Результат і докази ручного розрахунку'; required_pattern = 'Value'; require_external_focus = $false; expected_control_type = 'ControlType.Edit'; require_named_rows = $false; require_value_read_only = $true; allow_disabled = $true },
