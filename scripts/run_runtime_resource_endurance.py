...
def _seeded_leak_detector_check() -> tuple[bool, int]:
    ...
    try:
        ...
        detected = any(item.name == "autosport-resource-leak-sentinel" for item in residual)
        detected_count = len(residual)
    finally:
        release.set()
        thread.join(_WORKER_JOIN_TIMEOUT_SECONDS)
    ...
    restored = capture_runtime_resource_census()
    if residual_owned_threads(baseline, restored):
        raise RuntimeError("seeded leak negative control did not restore baseline")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for cycle in range(1, args.cycles + 1):
            ...
            current = capture_runtime_resource_census()
            ...
            if residual:
                failures.append(
                    ...
                    + ",".join(item.name for item in residual)
                )
        ...
        failures.append(f"qualification exception:{error_type}")

    final_census = capture_runtime_resource_census()
    final_residual = residual_owned_threads(baseline, final_census)
    if final_residual:
        failures.append(
            "final owned thread residual:"
            + ",".join(item.name for item in final_residual)
        )
    ...
    report: dict[str, object] = {
        ...
        "final_residual_threads": [item.to_dict() for item in final_residual],
