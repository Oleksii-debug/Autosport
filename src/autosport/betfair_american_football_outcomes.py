"""Bounded Betfair American Football terminal-outcome authority adapter.

This module extends the existing canonical ``market_outcomes`` authority issuance
boundary for Betfair American Football without creating a second outcome engine,
...
for an OPEN American Football MATCH_ODDS market whose runner roster is present in
the exact provider definition. It does not claim that every conservative state is
...
_BETFAIR_AMERICAN_FOOTBALL_EVENT_TYPE_ID = "6423"


def assess_betfair_american_football_historical_market_definition_authority(
    *,
    market_id: str,
    ...
    """Verify one Betfair American Football MATCH_ODDS roster."""
    ...
    if event_type_id != _BETFAIR_AMERICAN_FOOTBALL_EVENT_TYPE_ID:
    ...
    settlement_protocol = {
        "provider": _BETFAIR_SOURCE_ID,
        "sport": "american_football",
        "event_type_id": _BETFAIR_AMERICAN_FOOTBALL_EVENT_TYPE_ID,
    ...
    verification_protocol_sha256 = _sha256_payload(
        {
            "protocol": (
                "autosport.betfair_historical.american_football."
