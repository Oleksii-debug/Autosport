"""Bounded Betfair Ice Hockey terminal-outcome authority adapter.

This module extends the canonical ``market_outcomes`` authority issuance boundary
for Betfair Ice Hockey without creating a second outcome, settlement, provider,
or execution engine.
...
for an OPEN Ice Hockey MATCH_ODDS market whose exact runner roster is present in
the provider definition. It does not claim that every conservative state is
possible, that settlement rules are exact, that evidence is live/executable, or
that any economic edge exists.
"""
...
_BETFAIR_ICE_HOCKEY_EVENT_TYPE_ID = "7524"


def assess_betfair_ice_hockey_historical_market_definition_authority(
    *,
    market_id: str,
    ...
    """Verify one Betfair Ice Hockey MATCH_ODDS roster from historical evidence.

    Betfair identifies Ice Hockey with eventTypeId ``7524``. The adapter binds
    that provider identifier and exact ``MATCH_ODDS`` market type explicitly;
    display names are never used to infer sport or market semantics.

    Terminal outcomes use the canonical conservative Cartesian WIN/LOSS/VOID
    family. This proves coverage of the settlement alphabet represented by
    Autosport while deliberately retaining impossible combinations rather than
    pretending to have a tighter Ice-Hockey-specific settlement theorem.
    """
    ...
    if event_type_id != _BETFAIR_ICE_HOCKEY_EVENT_TYPE_ID:
    ...
    settlement_protocol = {
        "provider": _BETFAIR_SOURCE_ID,
        "sport": "ice_hockey",
        "event_type_id": _BETFAIR_ICE_HOCKEY_EVENT_TYPE_ID,
    ...
    verification_protocol_sha256 = _sha256_payload(
        {
            "protocol": (
                "autosport.betfair_historical.ice_hockey."
                "market_definition_roster.v1"
            ),
            "sport": "ice_hockey",
            "event_type_id": _BETFAIR_ICE_HOCKEY_EVENT_TYPE_ID,
