...
class BetfairMarketCommissionReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.commission, Decimal) or not self.commission.is_finite() or self.commission < 0:
            raise BetfairMarketCommissionAuthorityError("commission must be a finite non-negative Decimal")
        if not isinstance(self.profit, Decimal) or not self.profit.is_finite():
            raise BetfairMarketCommissionAuthorityError("profit must be a finite Decimal")
        if _CURRENCY_RE.fullmatch(self.currency) is None:
            raise BetfairMarketCommissionAuthorityError("currency must be uppercase three-letter code")
        for label, value in (
        ...
        if self.settled_at > self.available_at or self.observed_at > self.available_at:
            raise BetfairMarketCommissionAuthorityError("commission cannot be available before provider evidence")
        for label, value in (
    ...
    @classmethod
    def from_dict(cls, raw: Mapping[str, Any]) -> "BetfairMarketCommissionReceipt":
        _keys(
            raw,
            {
                "schema_version", "source_family", "venue_id", "account_id",
                "adapter_id", "adapter_version", "market_id", "commission", "profit",
                "currency", "settled_at", "observed_at", "available_at",
                "account_details_sha256", "cleared_orders_sha256", "request_scope_sha256",
        ...
        item = cls(
            ...
            account_details_sha256=_string(raw["account_details_sha256"], "account_details_sha256"),
            cleared_orders_sha256=_string(raw["cleared_orders_sha256"], "cleared_orders_sha256"),
            request_scope_sha256=_string(raw["request_scope_sha256"], "request_scope_sha256"),
        ...
        if raw["receipt_id"] != item.receipt_id or raw["record_sha256"] != item.record_sha256:
            raise BetfairMarketCommissionAuthorityError("Betfair commission receipt digest mismatch")
        return item
...
class BetfairMarketCommissionAuthority:
    ...
    No caller-provided transport, clock, currency, commission, profit, payload digest,
    or provider receipt is accepted. The authority creates its own integrated
    `BetfairReadOnlyClient`, reads authenticated account currency, and requests the
    official `listClearedOrders(groupBy=MARKET)` rollup for one market. Provider
    credentials are used only by the client and are never persisted.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._venue_id = _canonical_text(venue_id, "venue_id")
        self._account_id = _canonical_text(account_id, "account_id")
        ...
        self._authority = MonotonicWorkspaceAuthority(
            ...
            key=f"{self._venue_id}:{self._account_id}",
    ...
    def capture_market(
        ...
        *,
        ...
        if settled_from is not None:
            date_range["from"] = _canonical_provider_time(settled_from, "settled_from")
        if settled_to is not None:
            date_range["to"] = _canonical_provider_time(settled_to, "settled_to")
        if date_range:
            if "from" in date_range and "to" in date_range:
                if _parse_provider_time(date_range["from"], "settled_from") > _parse_provider_time(date_range["to"], "settled_to"):
                    raise BetfairMarketCommissionAuthorityError("settled_from cannot be after settled_to")
        ...
        try:
            ...
            raise BetfairMarketCommissionAuthorityError("Betfair MARKET commission acquisition failed") from exc
        ...
        if type(more_available) is not bool or more_available:
            raise BetfairMarketCommissionAuthorityError("Betfair MARKET commission capture is incomplete")
        rows = report.get("clearedOrders")
        if not isinstance(rows, list) or len(rows) != 1:
            raise BetfairMarketCommissionAuthorityError("exactly one MARKET rollup is required")
        row = _mapping(rows[0], "clearedOrders[0]")
        returned_market = _canonical_text(row.get("marketId"), "marketId")
        if returned_market != market:
            raise BetfairMarketCommissionAuthorityError("Betfair returned a different market")
        commission = _provider_decimal(row.get("commission"), "commission")
        if commission < 0:
            raise BetfairMarketCommissionAuthorityError("Betfair MARKET commission cannot be negative")
        profit = _provider_decimal(row.get("profit"), "profit")
        settled_at = _parse_provider_time(_string(row.get("settledDate"), "settledDate"), "settledDate")
        account_observed = _parse_provider_time(details.evidence.observed_at, "account observed_at")
        cleared_observed = _parse_provider_time(response.evidence.observed_at, "cleared observed_at")
        observed_at = max(account_observed, cleared_observed)
        available_at = observed_at
        records = self._load_records()
        predecessors = [value for value in records if value.market_id == market]
        predecessor = max(predecessors, key=lambda value: value.available_at, default=None)
        ...
        if predecessor is not None:
            same_evidence = (
                predecessor.cleared_orders_sha256 == candidate.cleared_orders_sha256
                and predecessor.account_details_sha256 == candidate.account_details_sha256
                and predecessor.request_scope_sha256 == candidate.request_scope_sha256
                and predecessor.commission == candidate.commission
                and predecessor.profit == candidate.profit
                and predecessor.currency == candidate.currency
                and predecessor.settled_at == candidate.settled_at
            )
            if same_evidence:
                return predecessor
            candidate = BetfairMarketCommissionReceipt(
        ...
        self._publish(tuple(sorted((*records, candidate), key=lambda value: value.receipt_id)))
        return candidate
    ...
    def resolve(
        ...
        *,
        ...
        if len(matches) != 1:
            raise BetfairMarketCommissionAuthorityError("Betfair commission receipt is not uniquely committed")
        receipt = matches[0]
        if receipt.record_sha256 != record_sha256:
            raise BetfairMarketCommissionAuthorityError("Betfair commission receipt digest mismatch")
        if receipt.available_at > as_of:
            raise BetfairMarketCommissionAuthorityError("future Betfair commission receipt cannot be backdated")
        for successor in records:
            if successor.supersedes_receipt_id == receipt.receipt_id and successor.available_at <= as_of:
                raise BetfairMarketCommissionAuthorityError("Betfair commission receipt was superseded before as_of")
        return receipt
    ...
    def _load_records(self) -> tuple[BetfairMarketCommissionReceipt, ...]:
        ...
        _keys(raw, {"schema_version", "records", "state_sha256"}, "Betfair commission state")
        if raw["schema_version"] != SCHEMA_VERSION:
            raise BetfairMarketCommissionAuthorityError("unsupported Betfair commission state schema")
        records = tuple(
        ...
        if tuple(sorted(records, key=lambda value: value.receipt_id)) != records:
            raise BetfairMarketCommissionAuthorityError("Betfair commission receipts are not canonically sorted")
        _validate_lineage(records)
        state_sha256 = _state_digest(records)
        if raw["state_sha256"] != state_sha256:
            raise BetfairMarketCommissionAuthorityError("Betfair commission state digest mismatch")
    ...
    def _publish(self, records: tuple[BetfairMarketCommissionReceipt, ...]) -> None:
        _validate_lineage(records)
        ...
        if prepared.phase is not AuthorityPhase.PREPARE or prepared.intended_state_sha256 != intended:
            raise BetfairMarketCommissionAuthorityError("Betfair commission state did not acquire exact PREPARE")
        payload = {
            "schema_version": SCHEMA_VERSION,
        ...
        observed = _strict_json(self._state_path().read_bytes(), "Betfair commission state")
        if observed.get("state_sha256") != intended:
            raise BetfairMarketCommissionAuthorityError("Betfair commission durable re-read failed")
...
def _validate_lineage(records: Sequence[BetfairMarketCommissionReceipt]) -> None:
    by_id = {value.receipt_id: value for value in records}
    if len(by_id) != len(records):
        raise BetfairMarketCommissionAuthorityError("duplicate Betfair commission receipt identity")
    ...
    for value in records:
        ...
        if prior is None:
            raise BetfairMarketCommissionAuthorityError("Betfair commission predecessor is missing")
        if predecessor in successors:
            raise BetfairMarketCommissionAuthorityError("Betfair commission receipt has multiple successors")
        if (
            ...
            raise BetfairMarketCommissionAuthorityError("Betfair commission correction rewrites source identity")
        if value.available_at < prior.available_at:
            raise BetfairMarketCommissionAuthorityError("Betfair commission correction predates predecessor")
    ...
    for start in by_id:
        ...
        while cursor is not None:
            if cursor in seen:
                raise BetfairMarketCommissionAuthorityError("Betfair commission lineage contains a cycle")
            seen.add(cursor)
            cursor = successors.get(cursor)


def _state_digest(records: Sequence[BetfairMarketCommissionReceipt]) -> str:
    return _digest({"schema_version": SCHEMA_VERSION, "records": [value.to_dict() for value in records]})


def _binding(state_sha256: str) -> str:
    return _digest({"domain": "autosport.betfair_market_commission.v1", "state_sha256": state_sha256})


def _canonical_text(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value or value != value.strip():
        raise BetfairMarketCommissionAuthorityError(f"{label} must be non-empty canonical text")
    return value
...
def _canonical_currency(value: Any) -> str:
    ...
    if _CURRENCY_RE.fullmatch(text) is None:
        raise BetfairMarketCommissionAuthorityError("Betfair account currency is not canonical")
    return text
...
def _parse_provider_time(value: Any, label: str) -> datetime:
    ...
    try:
        ...
        raise BetfairMarketCommissionAuthorityError(f"{label} is invalid") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise BetfairMarketCommissionAuthorityError(f"{label} must be timezone-aware")
    return parsed.astimezone(timezone.utc)


def _provider_decimal(value: Any, label: str) -> Decimal:
    if isinstance(value, bool) or not isinstance(value, (int, float, Decimal)):
        raise BetfairMarketCommissionAuthorityError(f"{label} must be provider numeric data")
    try:
        parsed = value if isinstance(value, Decimal) else Decimal(str(value))
    except InvalidOperation as exc:
        raise BetfairMarketCommissionAuthorityError(f"{label} is invalid") from exc
    if not parsed.is_finite():
        raise BetfairMarketCommissionAuthorityError(f"{label} must be finite")
    return parsed
...
def _sha256(value: str, label: str) -> None:
    if not isinstance(value, str) or _SHA256_RE.fullmatch(value) is None:
        raise BetfairMarketCommissionAuthorityError(f"{label} must be lowercase SHA-256 hex")
...
def _utc(value: datetime, label: str) -> None:
    ...
    if value.utcoffset() != timezone.utc.utcoffset(value):
        raise BetfairMarketCommissionAuthorityError(f"{label} must be UTC")
...
def _parse_decimal(value: Any, label: str) -> Decimal:
    ...
    if _decimal_text(parsed) != text:
        raise BetfairMarketCommissionAuthorityError(f"{label} is not canonical Decimal text")
    return parsed
...
def _parse_datetime(value: Any, label: str) -> datetime:
    ...
    if not text.endswith("Z"):
        raise BetfairMarketCommissionAuthorityError(f"{label} must use UTC Z notation")
    try:
        parsed = datetime.fromisoformat(text[:-1] + "+00:00")
    except ValueError as exc:
        raise BetfairMarketCommissionAuthorityError(f"{label} is invalid") from exc
    if _datetime_text(parsed) != text:
        raise BetfairMarketCommissionAuthorityError(f"{label} is not canonical")
    return parsed
...
def _keys(raw: Mapping[str, Any], expected: set[str], label: str) -> None:
    if set(raw) != expected:
        raise BetfairMarketCommissionAuthorityError(f"{label} keys mismatch")


def _string(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise BetfairMarketCommissionAuthorityError(f"{label} must be a string")
    return value
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, dict):
        raise BetfairMarketCommissionAuthorityError(f"{label} must be an object")
    return value


def _list(value: Any, label: str) -> list[Any]:
    if not isinstance(value, list):
        raise BetfairMarketCommissionAuthorityError(f"{label} must be an array")
    return value
...
def _strict_json(raw: bytes, label: str) -> Mapping[str, Any]:
    try:
        ...
        raise BetfairMarketCommissionAuthorityError(f"{label} is not UTF-8") from exc

    def no_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for key, value in pairs:
            if key in result:
                raise BetfairMarketCommissionAuthorityError(f"{label} contains duplicate key {key!r}")
            result[key] = value
        return result

    def reject_constant(value: str) -> Any:
        raise BetfairMarketCommissionAuthorityError(f"{label} contains non-standard number {value}")

    try:
        parsed = json.loads(text, object_pairs_hook=no_duplicates, parse_constant=reject_constant)
    except json.JSONDecodeError as exc:
        raise BetfairMarketCommissionAuthorityError(f"{label} is not valid JSON") from exc
    if not isinstance(parsed, dict):
        raise BetfairMarketCommissionAuthorityError(f"{label} must contain an object")
    return parsed
