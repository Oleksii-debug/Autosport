"""Bounded Betfair Soccer terminal-outcome authority adapter.

This module extends the existing canonical ``market_outcomes`` authority issuance
boundary for one additional Betfair sport.  It does not create a second outcome
engine, settlement engine, provider client, or execution authority.
...
for an OPEN Soccer MATCH_ODDS market whose runner roster is present in the exact
provider definition.  It does not claim that every conservative state is possible,
that the evidence is live/executable, or that any economic edge exists.
"""
...
_BETFAIR_SOCCER_EVENT_TYPE_ID = "1"


def assess_betfair_soccer_historical_market_definition_authority(
    *,
    market_id: str,
    ...
    """Verify one Betfair Soccer MATCH_ODDS roster from historical marketDefinition.

    Betfair's navigation contract identifies Soccer as eventTypeId ``1``.  The
    provider market-type code is bound explicitly to ``MATCH_ODDS``; display names
    are never used to infer sport or market semantics.

    The returned authority deliberately uses the canonical conservative Cartesian
    WIN/LOSS/VOID cover already consumed by ``ScenarioSearchEngine``.  Soccer gets a
    sport/event-type-bound settlement-rules digest and verification protocol, so a
    table-tennis rule witness cannot silently substitute for this adapter.
    """
    ...
    if event_type_id != _BETFAIR_SOCCER_EVENT_TYPE_ID:
        return MarketOutcomeAuthorityAssessment(
            identity=identity,
            status=OutcomeAuthorityStatus.REFUSED,
            authority=None,
            refusal_reason="betfair_event_type_has_no_verified_soccer_roster_protocol",
        )
    if provider_market_type != _BETFAIR_MATCH_ODDS_TYPE:
        return MarketOutcomeAuthorityAssessment(
            identity=identity,
    ...
    # Bind Soccer/eventTypeId explicitly.  The canonical result alphabet is shared
    # with the generic Betfair historical importer, but compatibility is asserted by
    # this adapter instead of being inferred from the MATCH_ODDS display/type label.
    settlement_protocol = {
        "provider": _BETFAIR_SOURCE_ID,
        "sport": "soccer",
        "event_type_id": _BETFAIR_SOCCER_EVENT_TYPE_ID,
    ...
    verification_protocol_sha256 = _sha256_payload(
        {
            "protocol": "autosport.betfair_historical.soccer.market_definition_roster.v1",
            "sport": "soccer",
            "event_type_id": _BETFAIR_SOCCER_EVENT_TYPE_ID,
    ...
    source_revision = (
        "betfair-soccer-market-definition:"
        + publish_raw
        + ":"
