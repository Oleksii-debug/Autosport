"""Bounded Betfair Tennis terminal-outcome authority adapter.

This module extends the existing canonical market_outcomes authority issuance
boundary for Betfair Tennis without creating a second outcome engine, settlement
engine, provider client, or execution authority.
...
for an OPEN Tennis MATCH_ODDS market whose runner roster is present in the exact
...
_BETFAIR_TENNIS_EVENT_TYPE_ID = "2"


def assess_betfair_tennis_historical_market_definition_authority(
    *,
    market_id: str,
    ...
    """Verify one Betfair Tennis MATCH_ODDS roster from historical evidence.

    Betfair identifies Tennis with eventTypeId 2. The adapter binds that provider
    identifier and exact MATCH_ODDS market type explicitly; display names are
    never used to infer sport or market semantics.
    ...
    if event_type_id != _BETFAIR_TENNIS_EVENT_TYPE_ID:
    ...
    verification_protocol_sha256 = _sha256_payload(
        {
            "protocol": (
                "autosport.betfair_historical.tennis.market_definition_roster.v1"
            ),
            "sport": "tennis",
            "event_type_id": _BETFAIR_TENNIS_EVENT_TYPE_ID,
