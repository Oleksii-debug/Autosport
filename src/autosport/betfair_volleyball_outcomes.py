"""Bounded Betfair Volleyball terminal-outcome authority adapter.

This module extends the canonical ``market_outcomes`` authority issuance boundary
for Betfair Volleyball without creating a second outcome, settlement, provider,
or execution engine.
...
for an OPEN Volleyball MATCH_ODDS market whose exact runner roster is present in
...
_BETFAIR_VOLLEYBALL_EVENT_TYPE_ID = "998917"


def assess_betfair_volleyball_historical_market_definition_authority(
    *,
    market_id: str,
    ...
    """Verify one Betfair Volleyball MATCH_ODDS roster from historical evidence.

    Betfair identifies Volleyball with eventTypeId ``998917``. The adapter binds
    ...
    pretending to have a tighter Volleyball-specific settlement theorem.
    """
    ...
    if event_type_id != _BETFAIR_VOLLEYBALL_EVENT_TYPE_ID:
    ...
    settlement_protocol = {
        "provider": _BETFAIR_SOURCE_ID,
        "sport": "volleyball",
        "event_type_id": _BETFAIR_VOLLEYBALL_EVENT_TYPE_ID,
    ...
    verification_protocol_sha256 = _sha256_payload(
        {
            "protocol": (
                "autosport.betfair_historical.volleyball.market_definition_roster.v1"
            ),
            "sport": "volleyball",
            "event_type_id": _BETFAIR_VOLLEYBALL_EVENT_TYPE_ID,
