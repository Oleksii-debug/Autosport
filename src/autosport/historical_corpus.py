...
def _json_object_bytes(payload: bytes, *, context: str, path: Path) -> dict[str, Any]:
    try:
...
def _json_object(path: Path, *, context: str) -> dict[str, Any]:
    ...
    return _json_object_bytes(payload, context=context, path=path)
...
def _governance_proof(path: Path) -> tuple[dict[str, Any], str]:
    payload = _read_bytes(path, context="governance proof")
    raw = _json_object_bytes(payload, context="governance proof", path=path)
    proof_sha256 = _sha256_bytes(payload)
...
def _snapshot(
    ...
    *,
...
) -> tuple[list[tuple[MarketEvent, dict[str, Any]]], dict[str, Any], str]:
    ...
    try:
        ...
        raise ValueError(f"snapshot market is not valid UTF-8: {market_path}") from exc
    ...
    return rows, evidence, evidence_sha256
...
def assemble_historical_corpus(
    ...
    *,
    ...
    proof, governance_proof_sha256 = _governance_proof(proof_path)
    ...
    for market_value, evidence_value in snapshot_pairs:
        ...
        rows, evidence, evidence_sha256 = _snapshot(
            market_path,
            evidence_path,
