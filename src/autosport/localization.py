...
_UK_UA = MappingProxyType(
    {
        ...
        "ui.status.research_plan.required_missing": (
            "План дослідження: обов’язковий; файл ще не вибрано."
        ),
        ...
        "ui.status.research_plan.replay_busy": (
            "План дослідження не можна змінювати під час економічного повтору."
        ),
        ...
        "ui.status.bank.pending": (
            "Віртуальний банк: оновлюється після повтору; робоча область: {workspace}"
        ),
        ...
        "ui.status.dataset.validation_busy": (
            "Перевірка набору даних уже виконується; дочекайтеся завершального результату."
        ),
        ...
        "ui.status.dataset.identity_mismatch": (
            "Набір даних не змінено: невідповідність ідентичності завершальної перевірки."
        ),
        "ui.status.dataset.summary": (
            "Набір даних: {name}; спорт={sport}; SHA ринку={market_sha}; "
            "SHA запечатаних результатів={results_sha}"
        ),
        ...
        "ui.status.live.dataset_busy": (
            "Поточний знімок відкладено: перевірка набору даних ще виконується."
        ),
        ...
        "ui.error.live.api_key_missing": (
            "AUTOSPORT_PARLAYAPI_KEY не задано в середовищі; виберіть публічний перегляд "
            "або задайте ключ до запуску програми"
        ),
        "ui.status.live.busy": (
            "Поточний знімок уже виконується; дочекайтеся завершення поточного запиту."
        ),
        "ui.status.live.running": (
            "Поточний знімок виконується у фоновому процесі; інтерфейс залишається доступним."
        ),
        "ui.status.live.read_only_running": (
            "Виконується живе спостереження лише для читання. PaperBook не змінюється."
        ),
        "ui.error.live.snapshot": "Помилка поточного знімка: {detail}",
        "ui.status.live.failed": (
            "Живе спостереження не оновлено; стан повтору/паперовий стан не змінено."
        ),
        ...
        "ui.status.windows.research_plan_recovery_busy": (
            "План дослідження не можна змінювати під час відновлення робочої області."
        ),
        "ui.status.windows.dataset_recovery_busy": (
            "Набір даних не можна змінювати під час відновлення робочої області."
        ),
        "ui.status.windows.live_recovery_busy": (
            "Поточний знімок відкладено: відновлення робочої області ще виконується."
        ),
        ...
        "ui.status.recovery.dataset_busy": (
            "Відновлення заблоковано: перевірка набору даних ще виконується."
        ),
        ...
        "ui.status.recovery.already_busy": (
            "Відновлення робочої області вже виконується; другий процес відновлення не запущено."
        ),
        ...
        "ui.status.recovery.state_unavailable": (
            "Відновлення робочої області не завершено; стан економічного сеансу недоступний."
        ),
        ...
        "ui.status.recovery.failed_until_fixed": (
            "Відновлення робочої області не завершено; економічний стан лишається недоступним, "
            "а новий повтор заблоковано до усунення причини."
        ),
        "ui.status.recovery.start_failed": (
            "Відновлення робочої області не запущено; стан сеансу лишається закритим при помилці "
            "до повторного успішного відновлення."
        ),
        "ui.status.recovery.running": (
            "Відновлення робочої області виконується у фоновому процесі; "
            "стратегія={strategy_id}{plan_identity}. Потік Tk/UIA/NVDA лишається доступним; "
            "повтор, живе спостереження, відновлення та елементи конфігурації заблоковано "
            "до завершального стану."
        ),
        "ui.log.recovery.started": (
            "Відновлення робочої області запущено у фоновому процесі; "
            "стратегія={strategy_id}{plan_identity}; робоча область={workspace}."
        ),
        "ui.error.recovery.worker": "Відновлення робочої області відхилено закрито при помилці: {detail}",
        "ui.status.recovery.blocked": (
            "Відновлення робочої області не завершено; цю економічну робочу область заблоковано "
            "для нового повтору до успішного відновлення."
        ),
        "ui.status.recovery.no_result": (
            "Процес відновлення робочої області завершився без завершального результату; "
            "ця робоча область лишається закритою при помилці."
        ),
        "ui.error.recovery.identity_mismatch": (
            "Невідповідність ідентичності завершального результату відновлення: "
            "очікувана робоча область={expected_workspace}, стратегія={expected_strategy_id}; "
            "отримана робоча область={received_workspace}, стратегія={received_strategy_id}."
        ),
        ...
        "ui.recovery.summary": (
            "Відновлення робочої області: узгоджено={reconciled}; "
            "перервано_незафіксованих={aborted_uncommitted}; невирішених={unresolved}; "
            "робоча область={workspace}"
        ),
        "ui.status.recovery.unresolved_ticket": (
            "Відновлення робочої області має невирішений запуск; стан економічного сеансу недоступний."
        ),
        "ui.status.recovery.unresolved_suffix": (
            ". Є невирішений успадкований запуск без достатнього доказу підсумку; "
            "економічний стан лишається прихованим, а повтор закритим при помилці для цієї робочої області."
        ),
        ...
        "ui.error.recovery.reopen": (
            "Повторне відкриття робочої області після відновлення відхилено закрито при помилці: {detail}"
        ),
        "ui.status.recovery.reopen_blocked": (
            "Узгодження відновлення завершено, але стан економічного сеансу лишається недоступним; "
            "новий повтор заблоковано до успішного відновлення/повторного відкриття."
        ),
        ...
        "ui.status.replay.dataset_busy": (
            "Паперовий повтор не запускається: перевірка набору даних ще виконується."
        ),
        "ui.info.replay.dataset_required": "Спочатку виберіть набір даних.",
        "ui.status.replay.already_busy": (
            "Паперовий повтор уже виконується; дочекайтеся його завершального стану."
        ),
        "ui.status.replay.live_busy": (
            "Поточний знімок ще виконується; паперовий повтор почнеться лише після його завершення."
        ),
        ...
        "ui.status.replay.recovery_busy": (
            "Паперовий повтор не запускається: відновлення робочої області ще виконується."
        ),
        "ui.status.replay.recovery_required": (
            "Паперовий повтор заблоковано закрито при помилці: поточна робоча область не має "
            "успішного завершального результату відновлення."
        ),
        ...
        "ui.status.replay.preparing_ticket": (
            "Паперовий повтор готується; стан попереднього економічного сеансу приховано "
            "до підтвердженого переходу."
        ),
        "ui.error.replay.teardown": (
            "Паперовий повтор не запущено закрито при помилці: не вдалося завершити попередній "
            "економічний сеанс; цільовий повтор не стартував, а точна проблемна робоча область потребує відновлення."
        ),
        "ui.status.replay.teardown_blocked": (
            "Паперовий повтор не запущено: не вдалося завершити попередній економічний сеанс; "
            "застарілий економічний стан приховано, а проблемну робочу область заблоковано до відновлення."
        ),
        ...
        "ui.status.replay.running": (
            "Повтор виконується у фоновому процесі; стратегія={strategy_id}{plan_identity}. "
            "Клавіатура, фокус, F6/F7/F8 і журнал залишаються доступними; закриття програми "
            "заблоковано до завершення економічної транзакційної межі."
        ),
        ...
        "ui.status.replay.error_ticket": (
            "Повтор завершився помилкою; стан економічного сеансу недоступний до відновлення."
        ),
        ...
        "ui.evaluation.reopen_failed": (
            "Оцінювання недоступне: повторне відкриття робочої області після повтору не пройшло "
            "закриту при помилці перевірку."
        ),
        "ui.error.replay.reopen": (
            "Повторне відкриття робочої області після повтору відхилено закрито при помилці: {detail}"
        ),
        "ui.status.replay.reopen_blocked": (
            "Завершальний стан повтору не можна безпечно підтвердити; цю економічну робочу область "
            "заблоковано закрито при помилці. Виконайте «Відновити робочу область» або Control+Shift+R "
            "перед наступним повтором у цій робочій області."
        ),
        ...
        "ui.status.tickets.replay_running": (
            "Повтор виконується; стан квитків оновиться після завершення транзакції."
        ),
        "ui.status.close.replay_busy": (
            "Паперовий повтор ще виконується. Закриття програми заблоковано до завершення економічної "
            "транзакційної межі, щоб процес не обірвав commit у довільній точці."
        ),
        "ui.status.close.live_busy": (
            "Поточний знімок ще виконується. Закриття програми заблоковано до завершального стану живого "
            "спостереження, щоб процес не приховав незавершену межу збереження ринку/стану джерела."
        ),
        ...
        "ui.price_truth.betfair_last_traded": (
            ...
            "виконувану котировку перевірено={executable}; відповідність паперового виконання "
            "перевірено={fill_fidelity}."
        ),
