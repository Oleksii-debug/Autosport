...
from .localization_product_cli import PRODUCT_CLI_UK_UA
...
_PRODUCT_CLI_COLLISIONS = set(PRODUCT_CLI_UK_UA).intersection(
    set(_WINDOWS_SHELL_UK_UA)
    | set(WINDOWS_SURFACE_CONTENT_UK_UA)
    | set(OWNER_ECONOMIC_AUTHORITY_UK_UA)
    | set(GUI_EVIDENCE_EXPORT_UK_UA)
)
if _PRODUCT_CLI_COLLISIONS:
    raise RuntimeError(
        f"localization v8 product CLI resources collide: {sorted(_PRODUCT_CLI_COLLISIONS)!r}"
    )
