...
_REQUEST_SCHEMA_ID: Final[str] = "ollama-chat-v1"
...
class OllamaModelAdapter:
    """Translate one canonical transient invocation to a local Ollama chat endpoint."""
    ...
    def invoke(
        ...
        *,
        ...
        if type(invocation) is not ModelInvocationInput:
            raise ModelInvocationError("invocation must be ModelInvocationInput")
        if (
        ...
        payload = json.dumps(
            {
                ...
                "messages": [{"role": "user", "content": invocation.input_text}],
                "stream": False,
                "think": False,
            },
            ensure_ascii=False,
