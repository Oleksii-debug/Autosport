...
def _validate_scenario_future_identity(
    ...
    for group in groups:
        for outcome in group.outcomes:
            ...
            if len(parts) != 3:
                continue
            market_identity = (parts[0], parts[1])
            market_first_observed = first_observed_market_times.get(market_identity)
            if market_first_observed is not None and market_first_observed > decision_time:
                raise ValueError(
                    "research scenario outcome market identity first appears after decision: "
                    f"{parts[0]}|{parts[1]}"
                )
