...
class SQLiteMarketStore:
    ...
    def replay_events_at_frozen_cutoff(self, *, as_of: str) -> list[MarketEvent]:
        """Return the exact durable history generation first frozen for as_of.

        The first read of a causal cutoff atomically binds that normalized UTC
        decision instant to the store's current product-owned append generation.
        Later appends can be audited through events() and can appear under a
        newly issued later cutoff, but they cannot rewrite this already-issued
        cutoff even when their event-carried clocks are backdated.
        """
