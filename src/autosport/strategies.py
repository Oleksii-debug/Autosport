...
_STRATEGIES: dict[str, tuple[StrategySpec, StrategyFactory]] = {
    ...
    "research-v1": (
        StrategySpec(
            ...
            label="Typed research pipeline",
            description=(
                "Deterministic paper-only research strategy. It consumes explicit causal "
                "research_signal metadata and routes it through ForecastRecord, critic, "
                "portfolio-impact and PaperRiskPolicy before opening a virtual ticket."
            ),
            agent_names=("market-mirror", "research-v1"),
            opens_paper_tickets=True,
        ),
        lambda: [MarketMirrorAgent(), ResearchSignalAgent()],
    ),
}
