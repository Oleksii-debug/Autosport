...
def test_stale_last_known_requires_prior_snapshot_identity() -> None:
    ...
    without_prior = classify_account_readback_degradation(
        _signal(kind=ReadbackFailureKind.STALE_CACHE, provider_code=None)
    )
    assert without_prior.state is ReadbackDegradationState.UNKNOWN
    _assert_never_positive(without_prior)
...
def test_unbounded_or_secret_shaped_tokens_are_rejected() -> None:
    with pytest.raises(ReadbackDegradationError):
