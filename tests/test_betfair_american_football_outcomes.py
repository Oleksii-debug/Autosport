...
from autosport.betfair_american_football_outcomes import (
...
class BetfairAmericanFootballOutcomeAuthorityTests(unittest.TestCase):
    ...
    def test_authority_is_provider_and_sport_bound(self):
        american_football = self._american_football()
        table_tennis = self._table_tennis()

        self.assertEqual(american_football.identity.sport, "american_football")
        self.assertEqual(american_football.selection_ids, ("101", "202"))
        self.assertEqual(american_football.terminal_state_count, 9)
        self.assertFalse(american_football.terminal_space_exact)
        self.assertTrue(
            american_football.source_revision.startswith(
                "betfair-american-football-market-definition:"
            )
        )
        self.assertNotEqual(
            american_football.identity.quote_key("101"),
            table_tennis.identity.quote_key("101"),
        )
        self.assertNotEqual(
            american_football.settlement_rules_sha256,
            table_tennis.settlement_rules_sha256,
        )
        self.assertNotEqual(
            american_football.verification_protocol_sha256,
    ...
    def test_same_provider_local_quote_identity_is_sport_bound_and_round_trips(self):
        ...
        american_football = MarketEvent.from_dict(
            {**base, "sport": "american_football"}
        )
        table_tennis = MarketEvent.from_dict({**base, "sport": "table_tennis"})
        self.assertNotEqual(american_football.quote_key, table_tennis.quote_key)
        self.assertNotEqual(american_football.dedupe_key, table_tennis.dedupe_key)
        self.assertEqual(
            MarketEvent.from_dict(american_football.to_dict()),
            american_football,
        )
    ...
    def test_terminal_cover_preserves_each_runner_as_a_selection(self):
        ...
        home_wins = next(
            state
            for state in american_football.terminal_states
        ...
        settlement = american_football.settlement_by_quote(home_wins)
        self.assertEqual(
            settlement[american_football.identity.quote_key("101")],
            "win",
        )
        self.assertEqual(
            settlement[american_football.identity.quote_key("202")],
            "loss",
        )
    ...
    def test_shares_generic_scenario_engine_without_cross_sport_alias(self):
        ...
        table_tennis = self._table_tennis()
        book = PaperBook("100")
        american_football_ticket = book.open_ticket(
            [
                TicketLeg(
                    "same-provider-event",
        ...
        report = ScenarioSearchEngine().analyse_authoritative(
            [american_football_ticket, table_tennis_ticket],
            [american_football, table_tennis],
    ...
    def test_wrong_sport_market_or_status_fail_closed(self):
        ...
        self.assertEqual(
            ...
            "betfair_event_type_has_no_verified_american_football_roster_protocol",
        )
    ...
    def test_durable_readback_requires_fresh_same_sport_source_reverification(self):
        ...
        restored = MarketSettlementOutcomeAuthority.from_dict(
            raw,
            verified_authority=self._american_football(),
        )
        self.assertEqual(restored, american_football)
