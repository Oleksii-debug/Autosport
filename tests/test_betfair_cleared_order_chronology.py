...
def test_cleared_order_settled_at_placement_is_chronologically_coherent() -> None:
    action = _action()

    evidence = _verify(
        action,
        settled_date="2026-09-21T18:00:01+00:00",
    )

    assert isinstance(evidence, VerifiedProviderEffectEvidence)


def test_cleared_order_settled_before_placement_is_rejected_at_read_boundary() -> None:
    action = _action()

    with pytest.raises(
        BetfairReadOnlyError,
        match="settled_date must not predate placed_date",
    ):
        _capture(
            action,
