...
def test_exact_prematch_match_odds_produces_only_three_winners_plus_all_void() -> None:
    ...
    assert authority.settlement_semantics is SettlementSemantics.EXCLUSIVE_SINGLE_WINNER_OR_ALL_VOID
    ...
    assert {result.value for _, result in authority.terminal_states[-1].settlements} == {"void"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("numberOfActiveRunners", 2, "betfair_live_shape_numberOfActiveRunners_not_exact"),
...
def test_market_specific_rules_are_required_and_digest_pinned() -> None:
    ...
    missing["rules"] = ""
    assert _assess(description=missing).refusal_reason == "betfair_market_information_rules_missing"

    changed = _description()
    changed["rules"] = MARKET_RULES + " changed"
    assert _assess(description=changed).refusal_reason == "betfair_market_information_rules_digest_changed"
...
def test_nonempty_clarification_fails_closed_even_when_base_rules_match() -> None:
    ...
    assert assessment.status is OutcomeAuthorityStatus.REFUSED
    assert assessment.refusal_reason == "betfair_market_information_has_unreviewed_clarifications"
...
def test_nonfootball_or_wrong_market_family_cannot_mint_exactness() -> None:
    assert _assess(event_type_id="2593174").refusal_reason == "betfair_event_type_is_not_football"

    description = _description()
    description["marketType"] = "OVER_UNDER_25"
    assert _assess(description=description).refusal_reason == "betfair_market_type_is_not_match_odds"

    description = _description()
    description["bettingType"] = "LINE"
    assert _assess(description=description).refusal_reason == "betfair_betting_type_is_not_odds"
...
def test_nonzero_bet_delay_is_not_admitted_as_prematch_exact() -> None:
    book = _book()
    book["betDelay"] = 5
    ...
    assert assessment.status is OutcomeAuthorityStatus.REFUSED
    assert assessment.refusal_reason == "betfair_market_has_nonzero_bet_delay"
