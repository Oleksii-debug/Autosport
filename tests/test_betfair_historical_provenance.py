...
class BetfairHistoricalProvenanceTests(unittest.TestCase):
    ...
    def test_snapshot_identity_is_file_order_independent(self) -> None:
        first = market_file(path="a.bz2", digest=SHA_A)
        second = market_file(path="b.bz2", digest=SHA_B)
        self.assertEqual(
    ...
    def test_delayed_stream_cannot_satisfy_live_read_forward(self) -> None:
        ...
        self.assertTrue(
            qualify_stream_stratum(capture, EvaluationStratum.DELAYED_FORWARD).qualified
        )
        self.assertFalse(
            qualify_stream_stratum(capture, EvaluationStratum.LIVE_READ_FORWARD).qualified
        )
    ...
    def test_live_stream_does_not_prove_execution_stratum(self) -> None:
        ...
        self.assertFalse(
            qualify_stream_stratum(capture, EvaluationStratum.SUPERVISED_EXECUTION).qualified
        )
    ...
    def test_enrichment_is_causal_only_after_its_observed_at(self) -> None:
        enrichment = LawfulEnrichmentEvidence(
