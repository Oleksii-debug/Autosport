...
def test_unknown_current_status_cannot_mint_effect_evidence() -> None:
    ...
    # A non-empty string is not enough provider-state authority.  An unknown
    # currentOrders status must fail closed before positive execution evidence can
    # be issued, rather than being preserved only as payload metadata.
    with pytest.raises(ProviderEvidenceError):
        _verify(action, status="PROVIDER_UNKNOWN_STATUS")
