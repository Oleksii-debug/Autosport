...
def test_manual_service_delegates_every_v1_formula_to_canonical_engine() -> None:
    ...
    _assert_same_result(service.odds_conversion("2.5"), engine.odds_conversion("2.5"))
    _assert_same_result(service.american_to_decimal_odds("+150"), engine.american_to_decimal_odds("+150"))
    _assert_same_result(service.fractional_to_decimal_odds("1", "2"), engine.fractional_to_decimal_odds("1", "2"))
    _assert_same_result(service.implied_probability("2"), engine.implied_probability("2"))
    _assert_same_result(service.multiplicative_devig({"home": "2", "away": "2.2"}), engine.multiplicative_devig({"home": "2", "away": "2.2"}))
    _assert_same_result(service.expected_return("0.6", "2", "10"), engine.expected_return("0.6", "2", "10"))
    _assert_same_result(service.paper_payout("10", "2.5"), engine.paper_payout("10", "2.5"))
    _assert_same_result(service.fractional_kelly("0.6", "2", fraction="0.5", cap="0.25"), engine.fractional_kelly("0.6", "2", fraction="0.5", cap="0.25"))
    _assert_same_result(service.performance_summary("10", "100", "1000"), engine.performance_summary("10", "100", "1000"))
    _assert_same_result(service.return_dispersion(["1", "2", "3"], sample=True), engine.return_dispersion(("1", "2", "3"), sample=True))
    _assert_same_result(service.normal_confidence_interval("0.5", "0.1", "1.96", assumption="normal_approximation_acknowledged"), engine.normal_confidence_interval("0.5", "0.1", "1.96", assumption="normal_approximation_acknowledged"))
    _assert_same_result(service.paper_parlay("10", ["2", "3"]), engine.paper_parlay("10", ("2", "3")))
    _assert_same_result(service.paper_parlay("10", ["2", "3"], probabilities=["0.5", "0.4"], probability_assumption="independent"), engine.paper_parlay("10", ("2", "3"), probabilities=("0.5", "0.4"), probability_assumption="independent"))
    _assert_same_result(service.finite_scenario_table({"win": "10", "lose": "-5"}, completeness="complete"), engine.finite_scenario_table({"win": "10", "lose": "-5"}, completeness="complete"))
    _assert_same_result(service.maximum_drawdown(["100", "90", "110", "80"]), engine.maximum_drawdown(("100", "90", "110", "80")))


def test_invalid_raw_numeric_text_is_rejected_before_engine_invocation(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_collection_limits_reject_before_later_values_or_engine_work(monkeypatch: pytest.MonkeyPatch) -> None:
    service = ManualCalculationService()
    def explode(*args, **kwargs):
...
def test_manual_identifier_and_enum_text_fail_closed_before_engine() -> None:
    ...
    with pytest.raises(ValueError, match="assumption must be exactly one of"):
        service.normal_confidence_interval("0", "1", "1.96", assumption="not_acknowledged")
    with pytest.raises(ValueError, match="probability_assumption"):
        service.paper_parlay("1", ["2", "2"], probabilities=["0.5", "0.5"], probability_assumption="dependent")
...
def test_default_numeric_arguments_also_cross_manual_input_boundary() -> None:
    ...
    _assert_same_result(service.expected_return("0", "2"), CalculationEngine().expected_return("0", "2", "1"))
    _assert_same_result(service.fractional_kelly("0", "2"), CalculationEngine().fractional_kelly("0", "2", fraction="1", cap="1"))
...
def test_manual_evidence_constructor_rejects_forged_truth_fields() -> None:
    ...
    with pytest.raises(ValueError, match="service_version"):
        ManualCalculationEvidence(service_version="forged-service", input_mode=valid.input_mode, result=valid.result, real_money_execution=valid.real_money_execution, evidence_sha256=valid.evidence_sha256)
    with pytest.raises(ValueError, match="input_mode"):
        ManualCalculationEvidence(service_version=valid.service_version, input_mode="product_quote", result=valid.result, real_money_execution=valid.real_money_execution, evidence_sha256=valid.evidence_sha256)
    with pytest.raises(ValueError, match="real_money_execution"):
        ManualCalculationEvidence(service_version=valid.service_version, input_mode=valid.input_mode, result=valid.result, real_money_execution=True, evidence_sha256=valid.evidence_sha256)
    with pytest.raises(ValueError, match="evidence_sha256"):
        ManualCalculationEvidence(service_version=valid.service_version, input_mode=valid.input_mode, result=valid.result, real_money_execution=valid.real_money_execution, evidence_sha256="0" * 64)
...
def test_manual_evidence_rejects_forged_inner_result_even_with_matching_outer_hash() -> None:
    ...
    matching_outer_hash = calculation_manual._sha256(calculation_manual._evidence_payload(forged_result))
    with pytest.raises(ValueError, match="result_hash"):
        ManualCalculationEvidence(service_version=valid.service_version, input_mode=valid.input_mode, result=forged_result, real_money_execution=False, evidence_sha256=matching_outer_hash)


@pytest.mark.parametrize("field", ["numeric_text_chars", "identifier_chars", "identifier_utf8_bytes", "collection_items"])
...
def test_manual_service_snapshots_stricter_boundary_against_late_relaxation() -> None:
    boundary = CalculationInputBoundary(CalculationInputLimits(numeric_text_chars=1))
    service = ManualCalculationService(input_boundary=boundary)
