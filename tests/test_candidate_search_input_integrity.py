...
class CandidateSearchInputIntegrityTests(unittest.TestCase):
    ...
    def test_delimiter_bearing_structured_event_identity_is_supported(self) -> None:
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].legs, (leg,))

    def test_delimiter_bearing_market_or_selection_suffix_is_supported(self) -> None:
        leg = self._leg(
            quote_key="event-1|win|ner|ali|ce",
            event_id="event-1",
            odds=Decimal("2"),
            probability=Decimal("0.5"),
        )
    ...
    def test_tied_economics_are_canonical_before_narrow_beam_truncation(self) -> None:
        ...
        self.assertEqual(
            tuple(leg.quote_key for leg in forward[0].legs),
            ("a|winner|x", "b|winner|x"),
        )
