...
def rows(champion, challenger):
    return [PairedLoss(str(i), c, h) for i, (c, h) in enumerate(zip(champion, challenger, strict=True))]
...
def test_promotes_only_when_all_gates_pass():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=10, min_effective_pairs=10, min_mean_improvement=0.5, min_win_rate=0.8, alpha=0.05),
    )
...
def test_exact_sign_test_is_one_sided_upper_tail():
    decision = evaluate_promotion(
        ...
        PromotionPolicy(min_pairs=5, min_effective_pairs=5, alpha=1.0 - 1e-12),
    )
...
def test_ties_excluded_from_sign_test_but_in_total_and_mean():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=3, min_effective_pairs=1, alpha=0.9),
    )
...
def test_tie_tolerance_is_symmetric_and_inclusive():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=3, min_effective_pairs=1, tie_tolerance=0.125, alpha=0.9),
    )
    assert decision.ties == 2
...
def test_empty_evidence_fails_closed_with_explicit_reasons():
    decision = evaluate_promotion([], PromotionPolicy(min_pairs=2, min_effective_pairs=1))
...
def test_duplicate_evaluation_ids_are_rejected():
    with pytest.raises(PromotionEvidenceError, match="duplicate evaluation_id"):
        evaluate_promotion(
            ...
            PromotionPolicy(min_pairs=2, min_effective_pairs=1),
        )
...
def test_generator_input_is_consumed_once():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=6, min_effective_pairs=6, alpha=0.1),
    )
...
def test_non_pair_row_rejected_without_coercion():
    with pytest.raises(PromotionEvidenceError, match="must be PairedLoss"):
        evaluate_promotion([("x", 1.0, 0.0)], PromotionPolicy(min_pairs=1, min_effective_pairs=1))
...
def test_mean_improvement_threshold_can_block_significant_win_rate():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=20, min_effective_pairs=20, min_mean_improvement=0.001, alpha=0.05),
    )
...
def test_win_rate_threshold_can_block_mean_improvement_driven_by_outlier():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=7, min_effective_pairs=7, min_mean_improvement=1.0, min_win_rate=0.5, alpha=1.0 - 1e-12),
    )
...
def test_large_sign_test_remains_finite_positive_for_extreme_upper_tail():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=2000, min_effective_pairs=2000, alpha=0.05),
    )
    assert decision.promote is True
...
def test_large_sign_test_lower_half_tail_is_near_one_without_underflow_failure():
    ...
    decision = evaluate_promotion(
        evidence,
        PromotionPolicy(min_pairs=2000, min_effective_pairs=2000, min_win_rate=0.0, alpha=0.999999),
    )
    assert decision.one_sided_sign_test_p_value is not None
...
def test_wrong_policy_type_rejected_explicitly():
    with pytest.raises(PromotionEvidenceError, match="policy must be PromotionPolicy"):
        evaluate_promotion([PairedLoss("x", 1.0, 0.0)], {"min_pairs": 1})
