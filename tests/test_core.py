...
class CoreTests(unittest.TestCase):
    def test_market_store_is_idempotent_orders_and_scopes_quote_identity(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            other = MarketEvent.from_dict({"event_id":"e2","market_id":"m","selection_id":"a","decimal_odds":"3.0","observed_ts":"2026-01-01T00:00:01+00:00","source_id":"s","sequence":2})
            self.assertEqual(store.append_many([first, first, second, other]), 3)
            current = store.current()
            self.assertEqual(current[first.quote_key].decimal_odds, Decimal("2.0"))
            self.assertEqual(current[other.quote_key].decimal_odds, Decimal("3.0"))
            store.close()
    ...
    def test_virtual_bank_settlement_and_restart_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
            book = PaperBook("100")
            leg = TicketLeg("e", "m", "a", Decimal("2.0"))
