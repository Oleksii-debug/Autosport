...
def _venue(source: str, account: str) -> VenueQuote:
    return VenueQuote(source, account, _quote(source), Decimal("50.00"))
    ...
    return ExecutionPlan(
        ...
        actions=tuple(
            ExecutionAction(
                ...
                expires_at="2026-09-21T19:00:00+00:00",
            )
...
def test_plan_reserved_actions_without_attempts_cannot_be_reissued(tmp_path) -> None:
    ...
    _assert_no_positive_reroute(venues=venues, ledger=ledger)

    # The supposedly "missing" action is not historical decoration: the canonical
    # ledger can still reserve its first attempt inside the persisted quote window.
    ledger.begin_attempt(
        plan_id=_PLAN_ID,
        action_id=initial.legs[0].leg_id,
        attempt_id="proof-attempt-a",
        reserved_at="2026-09-21T18:00:05+00:00",
    )
...
def test_unattempted_sibling_stays_blocked_after_other_child_is_accepted(
    ...
    outstanding_leg = initial.legs[1]
    ledger.begin_attempt(
        plan_id=_PLAN_ID,
        action_id=outstanding_leg.leg_id,
        attempt_id="proof-attempt-b",
        reserved_at="2026-09-21T18:00:05+00:00",
    )
