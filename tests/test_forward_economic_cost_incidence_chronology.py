...
def test_cost_evidence_cannot_precede_authoritative_incidence() -> None:
    obs = _observation(0)
    with pytest.raises(
        ForwardEconomicEvidenceError,
        match="cannot become available before cost incidence",
    ):
        _none(
            "challenger",
            obs,
            net="-60",
            cost="60",
            cost_incurred_at=T0 + timedelta(hours=6),
