...
def test_decision_commit_instants_are_normalized_to_canonical_utc() -> None:
    plus_two = timezone(timedelta(hours=2))
    step, _summary = _record(
        challenger_committed_at=datetime(2026, 9, 23, 8, 2, 3, tzinfo=plus_two),
        champion_committed_at=datetime(2026, 9, 23, 8, 2, 7, tzinfo=plus_two),
    )

    assert step.challenger_decision_committed_at.tzinfo is UTC
    assert step.champion_decision_committed_at.tzinfo is UTC
    assert step.challenger_decision_committed_at == datetime(
        2026, 9, 23, 6, 2, 3, tzinfo=UTC
    )
    assert step.champion_decision_committed_at == datetime(
        2026, 9, 23, 6, 2, 7, tzinfo=UTC
    )
    assert step.to_payload()["challenger_decision_committed_at"] == (
        "2026-09-23T06:02:03Z"
    )
