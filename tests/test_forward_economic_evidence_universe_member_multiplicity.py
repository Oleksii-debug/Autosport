...
def _protocol() -> ForwardEconomicProtocol:
    registry = FamilywiseAlphaRegistry(
        family_id="forward-family-member-multiplicity",
        total_alpha=Decimal("0.2"),
        allocations=(
            AlphaAllocation("challenger", Decimal("0.2")),
        ),
        sealed_at=T0,
    )
    return ForwardEconomicProtocol(
        protocol_id="forward-protocol-member-multiplicity",
        challenger_id="challenger",
        champion_id="champion",
        universe_id="universe-member-multiplicity",
...
def test_repeated_universe_member_cannot_mint_second_observation() -> None:
    accumulator = ForwardEconomicEvidenceAccumulator(_protocol())
    first = _observation(0, EVENT_A_SHA, "a" * 64, "b" * 64)
    repeated_member = _observation(1, EVENT_A_SHA, "c" * 64, "d" * 64)

    first_step = accumulator.record(first, _Resolver(first))
    before = accumulator.summary()
    duplicate_resolver = _Resolver(repeated_member)

    with pytest.raises(
        ForwardEconomicEvidenceError,
        match="cannot be counted more than once",
    ):
        accumulator.record(repeated_member, duplicate_resolver)
...
def test_distinct_universe_members_remain_independent_observations() -> None:
    accumulator = ForwardEconomicEvidenceAccumulator(_protocol())
    first = _observation(0, EVENT_A_SHA, "a" * 64, "b" * 64)
    second = _observation(1, EVENT_B_SHA, "c" * 64, "d" * 64)

    accumulator.record(first, _Resolver(first))
    accumulator.record(second, _Resolver(second))

    summary = accumulator.summary()
    assert summary.observed_events == 2
    assert summary.next_sequence == 2
