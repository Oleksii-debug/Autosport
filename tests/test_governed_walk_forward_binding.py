...
class GovernedWalkForwardBindingTests(unittest.TestCase):
    ...
    def test_outcome_perfect_backdated_forecasts_do_not_claim_verified_holdout_protocol(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            dataset = _write_governed_dataset(root / "dataset")
            raw = _raw_bundle(dataset)
            # This bundle can be authored after outcomes are known while declaring
            # syntactically valid pre-reveal timestamps. Without durable forecast-origin
            # evidence the evaluator must not promote those timestamps to protocol proof.
