    ...
    *event_ids: str,
    ...
    source_record.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "kind": "historical_outcome_source_record",
...
class HistoricalCorpusAssemblerTests(unittest.TestCase):
    def test_assembles_schema_v2_without_promoting_window_coverage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(outcome_evidence["source_record_outcomes_verified"])
    ...
    def test_changed_sealed_label_with_valid_source_hash_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            snapshot = _write_snapshot(root)
            results = _write_results(root, "tt-a")
            ...
            results.write_text(json.dumps(payload, sort_keys=True), encoding="utf-8")
            source_record = root / "official-outcome-source-record.json"
            provenance = payload["outcome_provenance"]
            self.assertEqual(provenance["source_record_sha256"], _sha256(source_record))

            output = root / "corpus"
            with self.assertRaisesRegex(ValueError, "do not match hashed outcome source record"):
                assemble_historical_corpus(
                    [snapshot],
                    results_path=results,
                    governance_proof_path=_write_governance(root),
