...
class HistoricalSnapshotSourceContractTests(unittest.TestCase):
    ...
    def test_coordinated_forged_provider_and_source_id_fail_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            market_path = root / "snapshot.jsonl"
            evidence_path = root / "snapshot.evidence.json"
            ...
            event = {
                "event_id": "tt-forged-source-contract",
                "market_id": "winner",
                "selection_id": "alice",
                "decimal_odds": "1.80",
                "observed_ts": "2026-01-01T10:00:01+00:00",
            ...
            with self.assertRaisesRegex(ValueError, "snapshot evidence provider must be parlayapi"):
                _snapshot(
                    market_path,
                    evidence_path,
                    expected_terms_reference="https://parlay-api.com/terms",
