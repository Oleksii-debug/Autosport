...
class IngestionHealthFailureBoundaryTests(unittest.TestCase):
    ...
    def test_post_persistence_subscriber_failure_records_exact_provider_success_then_reraises(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            provider = StaticProvider(
                "source",
                [
                    ProviderBatch("source", (first_quote,), cursor="1"),
                    # The duplicate proves provider health uses the bus/storage result
                    # (one durable acceptance), not len(normalized) (two inputs).
                    ProviderBatch("source", (first_quote, second_quote), cursor="2"),
                ],
            )
            ...
            self.assertIn("consumer failed after persistence", str(raised.exception.exceptions[0]))
            ...
            self.assertEqual(state.latest_source_ts, "2026-09-12T11:59:51+00:00")
