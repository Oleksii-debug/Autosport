"""Expected-RED falsifier for caller-mintable cloud permission.

This child intentionally consumes the canonical router test fixtures from the
parent lineage so it proves the public behavior without creating a second
router, VOC authority, or privacy registry.
"""
...
class CloudPermissionOriginTests(unittest.TestCase):
    def test_caller_flags_cannot_mint_positive_cloud_authority(self) -> None:
        ...
        # Expected RED on the current parent: caller-owned allow_cloud=True
        # and cloud_enabled=True still suffice for this permission dimension.
        # A safe successor may return LOCAL/DETERMINISTIC/WAIT, but it must not
        # issue CLOUD until it re-resolves a product-issued permission whose
        # backend/scope/revision was causally available for this decision.
