...
def test_same_instance_concurrent_path_registration_cannot_validate_two_roots(
    ...
    lexical paths while the stable selector store is still empty.  The current
    implementation then writes different O_EXCL path receipts, one per path.  Once
    that exact race outcome exists, normal revalidation must detect the split and
    must not allow both roots/namespaces to become authority-bearing.
    """
    ...
    # Exact first-registration precondition: both independent callers have already
    # scanned the same stable selector store before either path receipt exists.
    ...
    # Deterministically materialize the two writes that can follow those completed
    # prechecks.  O_EXCL cannot serialize them because each lexical path hashes to
    # a different receipt filename.
    _durable_exclusive_json_create(
    ...
    # One immutable workspace instance cannot legitimately validate under two
    # different physical roots.  Accepting both is the root-selection fork.
    ...
    # Different domain/key namespaces must not let both sides of the split become
    # independently activated after the shared workspace-instance fork exists.
