...
class ParticipantIdentityTests(unittest.TestCase):
    ...
    def test_roster_deduplicates_overlapping_same_entity_evidence_after_restart(self):
        registry = ParticipantIdentityRegistry.initialize_pristine(self.path)
        registry.add_entity(entity())
        first = RosterMembership("event-1", "provider-a", "p-1", T0, None, T0, SHA)
        second = RosterMembership("event-1", "provider-a", "p-1", T1, None, T1, "b" * 64)
        ...
        reopened = ParticipantIdentityRegistry(self.path)
        self.assertEqual(
            [item.entity_id for item in reopened.roster_at("event-1", "provider-a", as_of=T2)],
            ["p-1"],
        )
