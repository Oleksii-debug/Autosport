...
def test_registered_forecast_rejects_self_hashed_unissued_rating_snapshots(
    ...
    except ParticipantStrengthError:
        return
    except TypeError as exc:
        # A compatible repair may make the canonical rating authority/resolver
        # an explicit required argument. Do not allow an unrelated TypeError
        # to turn this falsifier green.
        message = str(exc).lower()
        assert any(
            token in message
            for token in ("opponent", "rating", "snapshot", "store", "resolver")
        ), message
        return

    pytest.fail(
        "caller-constructed self-hashed RatingSnapshot values minted a registered "
        f"participant-strength forecast without product-owned re-resolution: "
