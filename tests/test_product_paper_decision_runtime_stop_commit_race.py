...
class _StopWinsThenCommitLoop:
    ...
    def run_cycle(self):
        ...
        # These writes model the durable side effects that the canonical live loop
        # may perform after ProductPaperDecisionCycle's preflight.  A valid repair
        # must put a runtime-owned commit fence at/below this boundary rather than
        # merely discovering STOP after run_cycle() returns.
...
def test_runtime_stop_winning_before_commit_prevents_economic_mutation(
    ...
    with pytest.raises(
        ProductPaperDecisionCycleError,
        match="requires the canonical product runtime to be running",
    ):
