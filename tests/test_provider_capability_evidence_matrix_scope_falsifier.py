...
def _positive_write_fact(profile, integration):
...
def test_positive_capability_fact_cannot_qualify_across_matrix_scope(
    ...
    fact = _positive_write_fact(profile, integration)
    ...
    assert not second.qualifies(
        BookmakerCapability.PLACE_BET,
        accepted_grades=accepted,
        at_time=T3,
    ), "positive capability evidence must be bound to its environment/application scope"
