...
def test_exact_grant_allows_bounded_retention_at_boundary():
    auth = authority()
...
def test_forbidden_retention_allows_transient_use_but_rejects_persistence():
    auth = authority()
    transient = request(
        auth,
    ...
    assert decide_provider_output_use(auth, transient, decided_at=T1).allowed is True
