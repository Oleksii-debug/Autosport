...
class RealExecutionDecimalContextTests(unittest.TestCase):
    ...
    def test_extreme_exponent_action_fails_before_fixed_point_formatter(self) -> None:
        base = _plan().actions[0]
        for field, value in (
            ("requested_odds", Decimal("1e1000000")),
            ("requested_stake", Decimal("1e-1000000")),
        ):
            with self.subTest(field=field):
                hostile = replace(base, **{field: value})
                with patch.object(
    ...
    def test_extreme_exponent_ack_fails_before_fixed_point_formatter(self) -> None:
        acknowledgement = ExternalAcknowledgement(
            attempt_id="attempt-decimal-resource",
            external_receipt_id="receipt-decimal-resource",
            status=AcknowledgementStatus.PARTIAL,
            acknowledged_at="2026-09-21T20:00:03+00:00",
            accepted_odds=Decimal("2.5"),
            accepted_stake=Decimal("1e1000000"),
        )
    ...
    def test_bounded_exponent_decimal_preserves_existing_exact_semantics(self) -> None:
        base = _plan().actions[0]
        exponent_action = replace(
            base,
            requested_odds=Decimal("2.5e1"),
            requested_stake=Decimal("1e-3"),
        )
        payload = exponent_action.to_dict()

        self.assertEqual(payload["requested_odds"], "25")
        self.assertEqual(payload["requested_stake"], "0.001")
