...
def test_adapter_rejects_omitted_leading_canonical_folds() -> None:
    ...
    # The parent currently checks contiguity only from the first supplied fold.
    # Dropping the first real canonical fold therefore leaves (3, 4), which is
    # internally contiguous and reaches the final input, but is still incomplete.
    omitted_leading_fold = canonical.folds[1:]

    with pytest.raises(ReproducibilityManifestError):
        derive_walk_forward_splits(points, omitted_leading_fold)
