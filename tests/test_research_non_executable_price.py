...
class _FailIfCalledPipeline:
    ...
    def decide_and_open(self, **_kwargs):
        self.called = True
        raise AssertionError("economic research pipeline must not run for non-executable price evidence")


class ResearchNonExecutablePriceTests(unittest.TestCase):
    def test_explicit_non_executable_quote_fails_before_book_or_ledger_mutation(self) -> None:
        ...
        event = replace(
            base_event,
            metadata={**base_event.metadata, "execution_quote_verified": False},
        )
        ...
        forecast = ForecastRecord(
            ...
            model_id="non-executable-price-test",
            model_version="1.0.0",
            ...
            forecast_id="forecast-non-executable-price",
        )
        evidence = ResearchEvidence(
            evidence_id="evidence-non-executable-price",
            quote_key=event.quote_key,
        ...
        instruction = ResearchReplayInstruction(
            decision_id="decision-non-executable-price",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            context = AgentContext(
                ...
                replay_run_id="truth-boundary-test",
                decision_ledger=JsonlDecisionLedger(ledger_path),
            )

            with self.assertRaisesRegex(
                ValueError,
                "not verified executable price evidence",
            ):
