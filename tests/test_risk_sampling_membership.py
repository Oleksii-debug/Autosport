...
class RiskSamplingMembershipTests(unittest.TestCase):
    ...
    def test_requires_precommit_before_outcome_reveal(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = self._registry(
                Path(tmp),
                protocol_available="2026-09-10T00:00:01+00:00",
            )
            with self.assertRaisesRegex(
                RiskSamplingMembershipError,
                "not precommitted before outcome reveal",
            ):
                resolve_fixed_n_risk_membership(
                    path,
                    research_protocol_id=self.PROTOCOL_ID,
    ...
    def test_refuses_to_turn_membership_into_dependence_authority(self):
        for dependence in ("IID", "INDEPENDENT", "CALLER_ASSERTED"):
            with self.subTest(dependence=dependence), tempfile.TemporaryDirectory() as tmp:
                path = self._registry(
                    Path(tmp),
