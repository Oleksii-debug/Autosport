...
class ScheduledCycleCoverageTests(unittest.TestCase):
    def test_three_due_slots_require_three_exact_canonical_starts(self):
    ...
    def test_two_starts_in_one_due_slot_are_duplicate_execution(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = CollectorDeltaStore(Path(tmp) / "collector.db")
            schedule = _freeze(store)
            _start(store, "2026-01-01T00:00:10+00:00")
    ...
    def test_restart_reresolves_same_schedule_without_shifting_due_times(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = CollectorDeltaStore(Path(tmp) / "collector.db")
            first = _freeze(store)
            second = _freeze(
    ...
    def test_other_run_or_epoch_cannot_satisfy_same_source_slots(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = CollectorDeltaStore(Path(tmp) / "collector.db")
            schedule = _freeze(store)
            _start(
            ...
            self.assertFalse(coverage.schedule_coverage_complete)
            self.assertEqual(coverage.started_slot_count, 1)
            self.assertEqual(coverage.started_cycle_count, 1)
            self.assertEqual(
                coverage.missing_slot_ids,
                (schedule.slots[0].slot_id, schedule.slots[1].slot_id),
            )
    ...
    def test_schedule_commitment_ignores_query_scope_dict_order(self):
