...
class ContinuousSettlementTicketCausalityTests(unittest.TestCase):
    ...
    def test_outcome_available_at_ticket_placement_can_settle(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            book = PaperBook("100")
            leg = TicketLeg("event-1", "winner", "alice", Decimal("2"))
            ...
            settled, _evidence_ids = coordinator._settle(
            ...
            settled_book = PaperBook.load(root / "paper_book.json")
            self.assertIs(
                settled_book.tickets[ticket.ticket_id].status,
                TicketStatus.WON,
            )
            self.assertEqual(settled_book.balance, Decimal("110"))
