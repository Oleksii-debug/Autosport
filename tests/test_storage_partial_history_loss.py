...
class StoragePartialHistoryLossTests(unittest.TestCase):
    ...
    def test_missing_history_payload_witness_survives_redundant_projection_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "market.db"
            missing = self._event(
                selection_id="home",
            ...
            try:
                ...
                connection.execute(
                    "UPDATE current_quotes SET quote_key=? WHERE quote_key=?",
                    ("drifted-projection-key", missing.quote_key),
                )
                connection.commit()
                history_before = connection.execute(
                    "SELECT dedupe_key,quote_key,payload_json FROM market_events ORDER BY dedupe_key"
    ...
    def test_semantically_malformed_projection_decimal_remains_repairable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "market.db"
            event = self._event(
                selection_id="home",
            ...
            try:
                ...
                raw = json.loads(payload[0])
                raw["decimal_odds"] = "not-a-decimal"
                connection.execute(
            ...
            repaired = SQLiteMarketStore(db_path)
            try:
                self.assertEqual(repaired.current()[event.quote_key], event)
            finally:
                repaired.close()
